...
export function fillGlyphs(
  ...
  const partial =
    whole < cells && remainder > 0 ? (glyphs.partials[remainder - 1] ?? "") : "";
  // A started-but-tiny fraction still gets the thinnest visible sliver.
  const lead = partial === "" && whole === 0 && f > 0 ? (glyphs.partials[0] ?? "") : partial;
...
export function ProgressBar({
  ...
  const accent = variantColor(colors, thresholdVariant(fraction, variant, thresholds));
  const trackColor = tintTrack ? mix(accent, colors.border, 0.25) : colors.border;
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => setOwnFrame((f) => f + 1), 1000 / SWEEP_FPS);
    ...
    <box flexDirection="row" gap={1} alignItems="center">
      ...
      {thick ? (
        ...
        <box flexDirection="column">
          ...
          <text>
            {brackets ? <span fg={colors.border}> </span> : null}
            <span fg={accent}>{"▄".repeat((sweep ? sweep.lit : (runs?.filled ?? "")).length)}</span>
            <span fg={trackColor}>
              {"▄".repeat(
                Math.max(
                  0,
                  width - (sweep ? sweep.lit.length : (runs?.filled ?? "").length),
                ),
              )}
            </span>
          </text>
