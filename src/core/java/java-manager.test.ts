...
suite("chooseInstalled", () => {
	test("takes the highest installed JDK inside the window", () => {
		...
		expect(
			chooseInstalled(installed(8, 11, 17), { min: 8, max: 11 })?.major,
		).toBe(11);
	});
	...
	test("caps an unbounded requirement at the newest LTS", () => {
		...
		expect(
			chooseInstalled(installed(newest, newest + 1), { min: 21 })?.major,
		).toBe(newest);
	});
	...
	test("honours an explicit maximum above the LTS ceiling", () => {
		...
		const newest = LTS_MAJORS[0];
		expect(
			chooseInstalled(installed(newest + 1), { min: 21, max: newest + 1 })
				?.major,
		).toBe(newest + 1);
	});
