...
export class ForegroundRuntime implements RuntimeProvider {
  ...
  async stop(server: Server, options: StopOptions = {}): Promise<void> {
    ...
    if (owned) {
      try {
        ...
        logger.warn({ id: server.id, err: String(err) }, "could not write stop command");
      }
    ...
    logger.info({ id: server.id, pid: session.pid }, "stopping foreign session via SIGTERM");
    signal(session.pid, "SIGTERM");
    if (!(await waitForExit(session.pid, timeoutMs))) {
      logger.warn({ id: server.id, pid: session.pid }, "SIGTERM timed out; sending SIGKILL");
  ...
  async *logs(server: Server, options: LogOptions = {}): AsyncIterable<string> {
    ...
    const initial = options.tail === undefined ? lines : lines.slice(-options.tail);
