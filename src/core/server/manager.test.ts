...
describe("createServer", () => {
	test("installs, writes mctl.json, and registers the location", async () => {
		...
		const written = JSON.parse(
			await readFile(join(server.path, "mctl.json"), "utf8"),
		);
...
describe("editServer", () => {
	...
	test("sets and clears a Java pin", async () => {
		...
		expect(
			(await manager.editServer("pinning", { javaPin: null })).java,
		).toBeUndefined();
	});
...
describe("deleteServer", () => {
	...
	test("rejects an unknown server", async () => {
		await expect(manager.deleteServer("ghost")).rejects.toThrow(
			ServerOperationError,
		);
	});
});
