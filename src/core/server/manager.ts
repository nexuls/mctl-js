...
export class ServerManager {
	...
	async createServer(
		...
		if (!isAbsolute(dir)) {
			throw new ServerOperationError(
				id,
				`server path must be absolute: ${dir}`,
			);
		}
		...
		if (existing.some((entry) => entry.id === id)) {
			throw new ServerOperationError(
				id,
				`a server with id "${id}" already exists`,
			);
		}
	...
	async deleteServer(
		...
		if (server.state === "running") {
			throw new ServerOperationError(
				id,
				`server "${id}" is running; stop it first`,
			);
		}
		...
		logger.info(
			{ id, deletedFiles: options.deleteFiles === true },
			"deleted server",
		);
	}
	...
	async #install(
		...
		logger.info(
			{ id: plan.id, kind: plan.kind, minecraftVersion },
			"created server",
		);
		return server;
	}
...
async function moveIntoPlace(staging: string, dir: string): Promise<void> {
	...
	logger.info(
		{ staging, dir },
		"cross-device create; copying instead of renaming",
	);
